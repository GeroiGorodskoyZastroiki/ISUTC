...
namespace InputRebinder.Runtime
{
    ...
    public class InputRebinderBinding : MonoBehaviour
    {
        ...
        void Start()
        {
            if (PlayerPrefs.HasKey($"{MapName}{ActionName}{BindingIndex}Bind"))
                Action.asset.LoadBindingOverridesFromJson(PlayerPrefs.GetString($"{MapName}{ActionName}{BindingIndex}Bind"));
            ResetTextAndButtons();
        }
